...
function Home() {
  ...
  const handleFollowToggle = async (targetUser) => {
    ...
    try {
      ...
      else if (targetUser.account_type === "private") isRequest = true;

      const res = await axios.post(
        "http://127.0.0.1:8081/api/user/auth/follow",
  ...
  // 🔹 Auto-refresh following/request data every 5 seconds
  useEffect(() => {
    if (user?.id) {
      const token = localStorage.getItem("token");
    ...
    <div className="container mt-4">
      ...
      >
        ...
        <div className="d-flex gap-2">
          <button className="btn btn-light btn-sm fw-bold" onClick={handlePostClick}>
            📤 Upload Post
          </button>
          <button className="btn btn-light btn-sm fw-bold" onClick={handleProfileClick}>
            Profile
          </button>
            ...
            >
              <div className="card-body d-flex align-items-center">
                <img
                  src={
                    u.profile_pic
                      ? `http://127.0.0.1:8081/profile_pic/${u.profile_pic}`
                      : "/default-profile.png"
                  }
                  alt={u.name}
